...
# Recommended: conda env with JAX + Jumanji, e.g. `conda run -n adam python ppo_discrete.py`
...
# Set in `main()` before running jitted training (io_callback uses this).
LOGGER: Any = None
...
def flatten_observation_single(obs: Any) -> jax.Array:
    """Single-env observation (no batch axis) → (feat,) vector."""
    leaves, _ = jtu.tree_flatten(obs)
...
def make_train(config):
    ...
    def train(rng, exp_id):
        def train_setup(rng):
            # env reset → (state, timestep); agent sees timestep.observation
            rng, _rng_reset = jax.random.split(rng)
            _rng_resets = jax.random.split(_rng_reset, config["num_envs"])

            def _reset_env(key):
                s, ts = env.reset(key)
                return s, ts.observation

            state, obs_tree = jax.vmap(_reset_env)(_rng_resets)
            obs = flatten_observation_batched(obs_tree)
        ...
        def _train_loop(runner_state, unused):
            ...
            def _env_step(runner_state, unused):
                ...
                # reset env if needed (auto-reset style: start of step)
                def reset_if_done(obs, state, done, rng):
                    def _reset():
                        s, ts = env.reset(rng)
                        return s, flatten_observation_single(ts.observation)

                    def _keep():
                        return state, obs

                    return jax.lax.cond(done, _reset, _keep)

                rng, _rng_reset = jax.random.split(rng)
                rng_resets = jax.random.split(_rng_reset, config["num_envs"])
                state, obs = jax.vmap(reset_if_done)(obs, state, done, rng_resets)
                ...
                new_obs = flatten_observation_batched(ts.observation)
                reward = ts.reward
                new_done = ts.step_type == StepType.LAST
                info = ts.extras
