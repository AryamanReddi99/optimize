...
def scale_by_myano(
  ...
  This implements the second-moment update and normalized step of ANO:

  .. math::

    \begin{align*}
      m_k &\leftarrow \beta_1 m_{k-1} + (1 - \beta_1) g_k \\
      v_k &\leftarrow \beta_2 v_{k-1}
             - (1 - \beta_2) \,\mathrm{sign}(v_{k-1} - g_k^2)\, g_k^2 \\
      \hat{v}_k &\leftarrow v_k / (1 - \beta_2^k) \\
      u_k &\leftarrow
        \frac{|g_k| \, \mathrm{sign}(m_k)}
             {\sqrt{\hat{v}_k + \bar{\varepsilon}} + \varepsilon}.
    \end{align*}

  Args:
    b1: Decay rate for the exponentially weighted average of grads.
    b2: Decay rate for the exponentially weighted average of the ANO
      second-moment term.
